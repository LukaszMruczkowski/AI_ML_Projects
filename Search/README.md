...
Given two inputs, the initial state and the goal state, the AI will navigate to find the optimal solution of a series of actions

## Search Problem Terms

### Agent

The agent is an entity that perceives its environment and acts upon that environment. For instance, this entity could be an AI program, the environment could be the state of a chess game, and the AI will determine the action under that state, in other words, how to take the next move
...
The state from which the search algorithm starts, the environment when the agent first kicks in
...
i.e. result(s, a) return the state in s by acting as
...
The condition that determines whether a given state is a goal state, if a certain state passes the goal test, it means the algorithm should be terminated since we have found the solution

### Path Cost

A numerical cost associated with a given path </br>
The cost of each edge (between two vertices) could be 1 or more than 1 </br>
i.e. when navigating a path on the map, we do not only consider the distance but also try to minimize the path cost, which in real life is the traffic condition on certain roads. If there's a bad traffic on a certain road, then the cost of that road is high
...
Exhausts each direction before trying another direction. If there are several possible ways, the agent will choose randomly. So, DFS might not be efficient to find the path due to this randomness, and it does not guarantee to find the shortest path
...
Guarantee the shortest path, but takes relatively a long time as it seeks each direction step by step. Also, BFS only considers unweighted path and ignore the path cost, so it's more efficient to use A\* when doing map navigation rather than BFS
...
BFS typically uses a [queue](https://www.programiz.com/dsa/queue) data structure to store all of the possible actions. Below is the pseudo-code for the BFS implementation, the only difference between DFS and BFS is one uses the stack and one uses the queue
...
Check out the `examples` directory for Python implementation of DFS and BFS that solves mazes
...
heuristic function h(n). However, it only considers the distance between the current node to the destination but not the cost to reach this node from the source. The greedy algorithm does not guarantee the shortest path
...
Manhattan distance vs Euclidean distance. We use either of these to estimate the cost from the current state to the goal, then choose the action that minimizes the cost
...
Consider both the cost of the path until now and the estimated cost to the goal, g(n) + h(n)

- g(n) = cumulative cost reaching the current node
...
A recursive or backtracking algorithm that is used in decision-making and game theory. It provides an optimal move for the player assuming that the opponent is also playing optimally. For instance, in the figure below, the green player is trying to maximize the score and the red player is to minimize to score, which is the same in any game
...
For example, if we want to find the max of three nodes and we know the current node will be less than the previous node, then we do not need to explore every other subnode of it
...
A type of Minimax Algorithm that only searches to a certain depth, arrives at a decision more quickly
